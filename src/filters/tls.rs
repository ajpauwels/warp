//! Tls filters
//!
//! Filters that extract information about the remote tls certificate chain
...
) -> impl Filter<Extract = (Option<Vec<Vec<u8>>>,), Error = Infallible> + Copy {
    filter_fn_one(|route| futures::future::ok(route.peer_certificates()))
}
